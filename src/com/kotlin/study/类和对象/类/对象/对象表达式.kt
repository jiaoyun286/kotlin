...
/**
 * 有时，我们需要创建一个对某个类做了轻微改动的类的对象，而不用显式的声明新的子类。在JAVA中我们通常用匿名内部类来实现。
 * 而在Kotlin中我们用对象表达式。
 * 语法：object: 基类(){
 *   //要改变的实现
 * }
...
/**
 * 任何时候，我们如果只需要一个对象，而不需要任何特殊超类型，我们可以在object后面的大括号内写实现
 *
 */
fun foo(){
    val adHoc = object {
...
/**
 * 如果把匿名对象赋值给公有属性或作为公有方法的返回值，那么给公有属性或公有方法的返回值类型是这个匿名对象的超类类型
